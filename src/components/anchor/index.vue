...
export default {
    ...
    props:{
        ...
        labelName:{
            ...
            validator:(valueList) => {
                ...
                if(valueList.length !== setList.size){
                    throw new Error("Please check the 'labelName' [Array]， Like a [Object Set] features")
                } else {
                    return true
                }
            }
        ...
        lazy:{
            ...
            validator:(value) => {
                if((typeof value === 'string' && value === 'lazy') || typeof value === 'boolean' || (Array.isArray(value) && value.every(item => typeof item === 'number'))){
                    return true
                } else {
                    throw new TypeError("Please check the 'lazy' [Boolean] or [String('lazy')] or [Array(index : Number)]")
                }
            }
        }
    },
    ...
    methods:{
        initDefaultSoltComponents(){
            ...
            this.$slots.default.filter(item => item.elm)
            .map(item => {
        ...
        initCreatedLabel(){
            ...
            let labelList = document.querySelectorAll(".nav-line > [data-unique='nav-label']")
    ...
    render(h){
        ...
        const _injectLabel = () => {
            return Array.apply(null, { length: this.labelName.length }).map((item,index) => {
                return h(
                    "label",
                    {
                        attrs:{
                        ...
                        key:index,
                        /* on:{
                            "~click":_wakeUpLazyLoadComponent(index)
                        } */
                    },
                    [
       ...
       /*  const _wakeUpLazyLoadComponent = (active_index) => {
            active_index
        } */
        
        const _childComponentLazyLoadMarkInitFactory = () => {
            /**
             * @懒加载规则 
             * 
             * @param [Boolean]
            ...
            return ACTIONS[Object.prototype.toString.call(this.lazy)]()
        }
        return h(
            'div',              // 组件 wrapper
            {
                ...
                attrs:{
                    class:`full-vw-vh ${this.commonClassList.toString().replace(/,/g," ")}`
                },
                ref:"scroll-wrapper"
            },
            [
